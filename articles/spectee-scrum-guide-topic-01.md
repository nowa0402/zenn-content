...
属人化の回避、タスクの見える化だけにとどまらず、「３つの正式な成果物（プロダクトバックログ・スプリントバックログ・インクリメント）」全てにおいて透明性を担保する必要があるとされており、読む前と読んだ後とでは、この「透明性」ということを担保することの大変さの認識に違いが出てきていると感じている。チームにとどまらず、ステークホルダ全体に関連するものなのだと再認識した。
...
スクラムにおける５つのイベントは検査を支援する役割も担っていることが、スクラムガイドを呼んだ上で得た新鮮な知見だった。頻繁におこなう事に意味があり、そのための５つのイベントでのリズムではあるが、イベントが形骸化してしまう、いわゆる「ゾンビスクラム」にならないように注意したい。また、そもそも「ゾンビ」となってしまっていないかの検査及び適応も意識してやっていく必要があると学んだ。

適応にて触れている「許容範囲を逸脱している」ことを検査するためにも、デイリーなどのスクラムイベントにおいてそれを意識することも重要である。
...
今回、適応について皆で考えるに際し、「許容範囲を逸脱している」ことをどのように判断するのか、といった議論があり、そこで話し合った結果、逸脱には主に以下の４つの観点があることがわかった。

**「スケジュールの遅延」、「品質の問題」、「スコープの変更」、「リソースの問題」**

これらの観点を各スクラムイベントで検査することで、「成果となるプロダクトが受け入れられなかった」となる前に未然に防ぎ、迅速に適応へ映ることができる。

また、正しく迅速な適応を行うためにも、先に挙げられた、「検査」が大事であり、それを担保する「透明性」も必然的に重要となることを学ぶことができた。

#### 感想

基本的に「〜なければならない」「〜必要がある」等の強い口調で書かれている箇所は、だいたいこの3つを守らなければ達成できないようなことが書いてある。
それがスクラム初学者や、「三本柱」の存在がまだ定着していない人にとっては理不尽な捉えられ方をする傾向が今回のやっている中でも散見していたように思う。
そのため、スクラムをチーム、ないし組織内で広める際には三本柱のことを強く意識させることで、その後のスクラムガイドの読み込みや、共通単語として使えるため有効である。

また、ここでは触れていないが「スクラムの５つの価値基準」である「確約（Commitment）」「集中（Focus）」「公開（Openness）」「尊敬（Respect）」「勇気（Courage）」をチームが意識し体現することが、三本柱の実現と深く関わっている。
「三本柱」をチーム内に浸透させたい思っているのであれば、こちらも合わせて学習する必要があるだろう。

「三本柱」にしても、「スクラムの5つの価値基準」にしても、チームが活動する際の指針とすることで意思決定もやりやすくなるため、自分たちのチームがこれらから大きくハズレていないかは、定期的にチェック指定して行こうと思う。

その時に再びスクラムガイドにはお世話になるだろう。

#### アクション

- 透明性
  - 「ストーリーの詳細を気にしなくとも成果物を見るだけで内容を把握できる単位でストーリーを分解する」
    - また、ストーリーのレベルでなくとも、一つのタスクに対しても細分化し、必要があれば別タスクに分ける余地を設けることも行った
    - その結果、チーム内での障害の共有等がスムーズになり、問題解決までの時間が短くなった
- 検査
  - **「スケジュールの遅延」、「品質の問題」、「スコープの変更」、「リソースの問題」**
    - 各スクラムイベントにて、これらの観点を意識できるような問いかけを心がける
