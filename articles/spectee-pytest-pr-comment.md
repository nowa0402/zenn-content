...
プルリクエストのレビューの観点の一つに単体テストコードの妥当性チェックがあります。
その際の目安としてカバレッジを見ることがあります。
現状はローカル環境で実行してカバレッジを出力するか、GitHub Actionsの履歴を見る方法しかありませんでした。
レビューに必要な手順と時間が増えている問題が発生しています。

問題の解決として **pytestカバレッジをプルリクエストのコメントに表示する仕組みを導入** しました。
この仕組みの導入によって開発プロセスの透明性と効率性を向上させることができます。
