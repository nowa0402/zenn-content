...
まず、静的解析が目指す最終目的という抽象的な目的について定義します。
これは **「プロジェクトの持続可能性を高める」** です。
...
また、書籍：[GitHub CI/CD実践ガイド](https://gihyo.jp/book/2024/978-4-297-14173-8)では、CI/CDの目的を以下のように定義しています。
...
両例とも **「持続可能性」** を目的に掲げています。
このことから、「静的解析」も単体テストやCIと同じような目的を持っていると考えられます。

次に、静的解析がプロジェクトの持続可能性を高めるための具体的な目的を定義します。
これは **「開発者全員がコード品質を一定に保つための仕組み」** を提供することです。

ソフトウェア開発において、コードの品質が常に一定に保たれていることは、開発者が本質的な作業に集中し、プロジェクトの持続的な成長を促進するために重要であると考えています。そのための取り組みの一つとして「静的解析」が挙げられます。
...
- セキュリティの強化
  - 未然に脆弱性をスキャンし対応することが可能になる

これらを達成する仕組みの整備と見直しを行っていくのが本記事になります。

## 見直しポイント

静的解析で達成したい目的と現状の設定を踏まえた見直しポイントは以下の通りです。
